...
const UserITReturns = () => {
  ...
  const fetchITReturns = async () => {
    ...
    try {
      ...
      setITReturns(response.data.itReturns);
      setFilteredITReturns(response.data.itReturns);
    } catch (error) {
      if (
        isMounted &&
        error.response &&
  ...
  const handleSearch = (event) => {
    ...
    const filtered = itReturns.filter((itReturn) => {
      return (
        itReturn.files[0].filename
          .toLowerCase()
          .includes(event.target.value.toLowerCase()) ||
        itReturn.description
          .toLowerCase()
          .includes(event.target.value.toLowerCase()) ||
        itReturn.remarks
          .toLowerCase()
          .includes(event.target.value.toLowerCase()) ||
        itReturn.name.toLowerCase().includes(event.target.value.toLowerCase())
      );
    });
    setFilteredITReturns(filtered);
  };
  ...
  const currentItems = filteredITReturns.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const totalPagesC = Math.ceil(filteredITReturns.length / itemsPerPageC);
            ...
            <div className="bg-white shadow-lg rounded-lg overflow-hidden w-full md:w-11/12 lg:w-11/12  xl:w-11/12  mx-auto">
              <div className="p-5">
                ...
                <p>
                  ...
                  {selectedITReturn.description}
                </p>
                <p>
                  <strong className="text-gray-600">Remarks:</strong>
                  {selectedITReturn.remarks}
                </p>
                <p>
                  <strong className="text-gray-600">
                    Name of the Uploader:
                  </strong>
                  {selectedITReturn.name}
                </p>
                <p>
                  <strong className="text-gray-600">IT Returns Type:</strong>{" "}
                  {selectedITReturn.selectedReturnType}
                </p>
                <p>
                  <strong className="text-gray-600">
                    Email of the uploader:
                  </strong>{" "}
                  {selectedITReturn.email}
                </p>
                <p>
                ...
                <div className="flex items-center mt-4 flex-wrap">
                  {selectedITReturn.files[0].filename
                    ...
                    <button
                      ...
                      disabled={
                        loadingPreview[selectedITReturn.files[0].filename] ||
                        !selectedITReturn.files[0].filename.endsWith(".pdf")
                      }
                  ...
                  <button
                    ...
                    disabled={
                      loadingDownload[selectedITReturn.files[0].filename]
                    }
                      ...
                      <tr key={itReturn._id}>
                        ...
                        <td className="border px-4 py-2 text-center">
                          {itReturn.files[0].filename
                            ...
                            <button
                              ...
                              disabled={
                                loadingPreview[itReturn.files[0].filename]
                              }
                              className="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded"
                            >
                              {loadingPreview[itReturn.files[0].filename]
                        ...
                        <td className="border px-4 py-2 text-center">
                          <button
                            ...
                            disabled={
                              loadingDownload[itReturn.files[0].filename]
                            }
                            className="bg-transparent hover:bg-green-500 text-green-700 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded"
                          >
                            {loadingDownload[itReturn.files[0].filename]
